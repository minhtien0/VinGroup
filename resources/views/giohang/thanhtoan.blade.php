    ...
    <form action="{{ route('xulythanhtoan') }}" method="POST">
        ...
        <div class="voucher">
            ...
            <select name="voucher_id">
                ...
                <option value="{{ $voucher->id }}">{{ $voucher->code }} - Giảm {{ $voucher->discount }}đ</option>
                @endforeach
            </select>
        </div>

        <div class="total">
            <h3>Tổng tiền: {{ number_format($selectedItems->sum(function ($item) { return $item->product->price * $item->soluong; })) }}đ</h3>
        </div>
        <!-- Nút thanh toán -->
